...
func NewMongoService[DocType interface{}](config MongoServiceConfig) DbService[DocType] {
    ...
    if svc.ServerHost == "" {
        svc.ServerHost = enviro("BLOOD_DONORS_API_MONGODB_HOST", "localhost")
    }

    if svc.ServerPort == 0 {
        port := enviro("BLOOD_DONORS_API_MONGODB_PORT", "27017")
    ...
    if svc.UserName == "" {
        svc.UserName = enviro("BLOOD_DONORS_API_MONGODB_USERNAME", "")
    }

    if svc.Password == "" {
        svc.Password = enviro("BLOOD_DONORS_API_MONGODB_PASSWORD", "")
    }

    if svc.DbName == "" {
        svc.DbName = enviro("BLOOD_DONORS_API_MONGODB_DATABASE", "cv2xvancoa-blood-donors")
    }

    if svc.Collection == "" {
        svc.Collection = enviro("BLOOD_DONORS_API_MONGODB_COLLECTION", "donation_sites")
    }

    if svc.Timeout == 0 {
        seconds := enviro("BLOOD_DONORS_API_MONGODB_TIMEOUT_SECONDS", "10")
